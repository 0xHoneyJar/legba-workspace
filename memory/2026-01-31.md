...
Created framework for delineating improvements:

**Loa = WHAT + WHY (methodology)**
- Skill definitions, phase orchestration, handoff validation
- Quality criteria, feedback schema, escalation criteria
- Runtime-agnostic: should work on ANY runtime

**Runtime = HOW + WHERE (execution)**
- Token management, memory persistence, session management
- Tool execution, message routing, crash recovery
- Framework-agnostic: should support ANY methodology

**Integration = Contract**
- Exit codes (shared meaning)
- Checkpoint schema (shared format)
- Context signals (runtime → loa)

**Test questions:**
- "Would this apply to ANY agent using Loa?" → Loa
- "Would this apply to ANY workflow on this runtime?" → Runtime
- Both? → Integration (PRs to both repos)

Document: `/root/clawd/knowledge/SEPARATION-OF-CONCERNS.md`

## New PR Created: #78 Separation of Concerns

**PR #78**: https://github.com/0xHoneyJar/loa/pull/78

Documents:
1. `docs/architecture/separation-of-concerns.md`
   - Three-layer model diagram
   - Decision framework with test questions
   - Feature delineation examples

2. `docs/integration/runtime-contract.md`
   - Exit code handling requirements
   - Checkpoint schema (TypeScript)
   - Context signals interface
   - Escalation protocol
   - Implementation checklist

This extracts architectural guidance from PR #73 comments into proper living documentation.

Also created PRD at `grimoires/separation-of-concerns/prd.md` following Loa methodology.
